...
void gwPerson::updateLocusAttributes(const vectorF & proportionsMissingData, bool shouldMarkMissing, gsl_rng * gslr)
{
	...
	for (UINT i = 0; i != __locusAttributes.size(); ++i) {
		...
		if (__locusAttributes[i] == D_RV || __locusAttributes[i] == D_CV) {
			...
			std::clog << "WARNING: __locusAttributes appears to have been updated. Please make sure you do not call 'updateLocusAttributes(const vectorF& proportionsMissingData, bool shouldMarkMissing, gsl_rng* gslr)' multiple times. " << std::endl;
	}

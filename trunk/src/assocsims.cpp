...
//!-# "sibpair-odds", this is so confusing, I hate much overloading ...
void gwSimulator::createGenotypeComplexTraitsAssociations(const vectorF & propFunctionalRv, double baselinef,
                                                          const vectorF & oddsRatios, const char moi, unsigned nPairs, gsl_rng * gslr, const vector2UI & poolDat)
{
	//!- Initialize a "template" person object
	gwPerson * personTemplate = new gwPerson(__boundary, __neutral_cutoff, __pedInfos, __mafs, __fnctAnnotations, __positions);
	...
	bool isInputOk = (nPairs > 0);
	if (isInputOk) ;
	else {
		std::cerr << "Sibpair size not valid. Nothing to do. Quit now." << std::endl;
		exit(-1);
	}

	//!- Generate sibpair samples (resample, reject or accept)
	unsigned iPairs = 0;
	...
	while (iPairs != nPairs) {
		...
		if (trait == AFFECTED) {
			...
			__persons.push_back(*personTemplate);
			while (true) {
				// 25% that the sib is completely different
				// 25% that the sib is identical
				// 50% that the sib shares one genotype
				...
				if (runif < 0.25) {
					...
					double tmp = personTemplate->computeGenotypicEffect(oddsRatios, baselinef, moi);
					personTemplate->generatePhenotype(tmp, 0, gslr);
				} else {
					vector2F fgenotype = personTemplate->getGenotype();
					unsigned which = gsl_ran_bernoulli(gslr, 0.5);
					if (poolDat.size() > 0) {
						unsigned idx = gsl_rng_uniform_int(gslr, poolDat.size());
						for (unsigned i = 0; i < __mafs.size(); ++i) {
							fgenotype[which][i] = double(int(poolDat[idx][i]) % 2);
						}
					} else {
						for (unsigned i = 0; i < __mafs.size(); ++i) {
							fgenotype[which][i] = double(gsl_ran_bernoulli(gslr, __mafs[i]));
						}
					}
					personTemplate->updateGenotype(fgenotype);
					double tmp = personTemplate->computeGenotypicEffect(oddsRatios, baselinef, moi);
					personTemplate->generatePhenotype(tmp, 0, gslr);
				}
				double sibtrait = personTemplate->getPhenotype();
				if (sibtrait == UNAFFECTED) {
					__persons.push_back(*personTemplate);
					++iPairs;
					break;
				}
...
void gwSimulator::createPedfileMatrix(bool isSynoTrimmed, bool isCvTrimmed,
                                      bool isPedWritten, std::string projectName, std::string simulationTask)
{
